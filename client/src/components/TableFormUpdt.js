...
const TableFormUpdt = ({
  ...
  //Edit  Function
  const editRow = (orderdetails_id) => {
    const editingRow = list.find(
      (row) => row.orderdetails_id === orderdetails_id
    );
    setList(list.filter((row) => row.orderdetails_id !== orderdetails_id));
    setIsEditing(true);
  ...
  const deleteRow = (orderdetails_id) => {
    setList(list.filter((row) => row.orderdetails_id !== orderdetails_id));
  };
            ...
            <React.Fragment key={orderdetails_id}>
              <tbody>
                <tr>
                  ...
                  <td>
                    <button onClick={() => editRow(orderdetails_id)}>
                      <FaEdit className="text-green-500 font-bold text-xl" />
                    </button>
                  </td>
                  <td>
                    <button onClick={() => deleteRow(orderdetails_id)}>
